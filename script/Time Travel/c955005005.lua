...
function s.cfilter(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(SET_TIME_TRAVEL) and c:IsSummonLocation(LOCATION_REMOVED)
		and Duel.IsExistingMatchingCard(s.spfilter,tp,LOCATION_REMOVED,0,1,nil,e,tp,c:GetOriginalLevel())
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		...
		local sg=Duel.SelectMatchingCard(tp,s.spfilter,tp,LOCATION_REMOVED,0,1,1,nil,e,tp,tc:GetOriginalLevel())
		if #sg>0 then
