...
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return Duel.SelectMatchingCard(tp,Card.IsMonster,tp,LOCATION_HAND+LOCATION_MZONE,0,1,1,nil) end
	local tc=Duel.SelectMatchingCard(tp,Card.IsMonster,tp,LOCATION_HAND+LOCATION_MZONE,0,1,1,nil)
	if chk==0 then return Duel.IsExistingTarget(s.tgfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingMatchingCard(s.desfilter,0,0,LOCATION_MZONE,1,nil,tc) end
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not (tc:IsMonster() and tc:IsRelateToEffect(e)) then return end
