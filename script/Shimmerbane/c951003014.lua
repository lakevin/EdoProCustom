...
function s.initial_effect(c)
	...
	c:RegisterEffect(e2)
	-- (2) Can be treated as a non-tuner
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingTarget(s.actfilter,tp,LOCATION_SZONE,0,1,c) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id,SET_SHIMMERBANE,0x21+0x1000,1000,1000,4,RACE_FIEND,ATTRIBUTE_DARK) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0
		or not Duel.IsPlayerCanSpecialSummonMonster(tp,id,SET_SHIMMERBANE,0x21+0x1000,1000,1000,4,RACE_FIEND,ATTRIBUTE_DARK) then return end
	c:AddMonsterAttribute(TYPE_EFFECT+TYPE_TUNER,ATTRIBUTE_DARK,RACE_FIEND,4,1000,1000)
